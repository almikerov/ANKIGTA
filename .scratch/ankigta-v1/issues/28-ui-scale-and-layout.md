...
- [x] `Reset UI layout` is always visible/reachable. Две дороги, и вторая не зависит от первой: строка в панели настроек — тест приводит раскладку в худшее состояние на каждом разрешении и масштабе и убеждается, что кнопка на экране, внутри своего окна и работает, — и команда `/ankigta-ui-reset`, которой окно не нужно вовсе. Панель раскладывает то самое, что сбрасывают, поэтому путь назад не может опираться только на неё.
...
- Client UI settings (строки панели настроек тикета 27, не отдельное окно).
...
**Одна панель, а не две.** Этот тикет написал `client/ui_settings.lua` —
маленькое окно с масштабом, `Edit HUD layout` и `Reset UI layout`, — пока
тикет 27 параллельно строил панель настроек, в которой `uiScale` и так была
строкой из схемы. Два окна, предлагающих одно значение, — это два места, куда
игрок может пойти за одним и тем же, и два места, которые разойдутся. При
слиянии окно убрано: три элемента, которые не были настройками (шаги ±0.05,
`Edit HUD layout`, `Reset UI layout`), стали блоком под строкой `uiScale` в
панели настроек, а сама панель переехала на менеджер раскладки, как все
остальные окна. `/ankigta-ui` и кнопка в F7 открывают её же.
