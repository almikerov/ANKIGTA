"""Ticket 07 — ANKIGTA speaks English, and carries nothing for speaking two.
...
- the windows are rendered and the controls are read back, so a string that
  never reaches a control cannot pass by being present in the table.
"""
...
def named_keys(script: Path) -> set[str]:
    ...
    filename, and an MTA ACL right. A constant that ends in a dot is half of a
    key the script builds at runtime -- `"f7.linkState." .. state` -- and the
    values completing it are enumerated by whichever test owns that family.
    """
    return {
        value
...
def test_the_panel_is_given_every_key_it_renders() -> None:
