...
local function renderSnapshot(snapshot)
    ...
    -- One of the two ways to `Reset UI layout`. The other is a chat command,
    -- because "always reachable" has to hold even when this window is the one
    -- that is the wrong size.
    local uiSettingsButton = surface.button(
        352,
        height - 74,
        170,
        26,
    ...
    addEventHandler("onClientGUIClick", uiSettingsButton, function()
        if type(openUiSettings) == "function" then
            openUiSettings()
        end
    end, false)
